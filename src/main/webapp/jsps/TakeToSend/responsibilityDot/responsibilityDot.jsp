...
<body>
				...
				<div class="modal-body">
					<form class="form-horizontal" onsubmit="return check()" role="form" id="addForm">
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select id="addfranchiseeCode" class="yuan clean" style="width:210px;height:25px">
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'responsibilityDot/responsibilityDotQuery', //请求后台的URL（*） 
				method : 'post', //请求方式（*） 
				...
				uniqueId : "responsibilityDotId", //每一行的唯一标识，一般为主键列 
				...
				columns : [ {
					...
					formatter:function (value){
						...
						$('#btn_query').click(function() {
							var opt = {
								url : "responsibilityDot/responsibilityDotQueryWithCriteria",
								silent : true,
								query : {
									type : 1,
									level : 2,
					...
					var ButtonInit = function() {
						...
						oInit.Init = function() {
							...
							$("#btn_add").click(function() {
								$("#add").modal('show');
								$.ajax({
									url : "responsibilityDot/findOrganId",
									type : "post",
									traditional : true,
									success : function(response) {
									var select = $("#addfranchiseeCode");
								...
								$.ajax({
									url : "responsibilityDot/findUser",
									type : "post",
									traditional : true,
									success : function(response) {
										$("#addoperator").val(response);
								  }
							  });
							});
						...
						if (select.length != 1) {
							...
							$("#subUp").click(function() {
								...
								$.ajax({
											url : "responsibilityDot/updateresponsibilityDot",
											type : "post",
											data : {
												"responsibilityDotId" : select[0].responsibilityDotId,
						...
						if (select.length === 0) {
							...
							for (var i = 0; i < select.length; i++) {
								arrs[i] = select[i]['responsibilityDotId'];
							}

							$('#tb_departments').bootstrapTable('remove', {
								field : 'responsibilityDotId',
								values : arrs
							});
							$.ajax({
								url : "responsibilityDot/deleteresponsibilityDot",
								type : "post",
								data : {
									"responsibilityDotId" : arrs
								},
								traditional : true,//这里设置为true
								success : function(result) {
									if (result == "success") {
										toastr.success("删除成功");
										var url = {
											url : 'responsibilityDot/responsibilityDotQuery'
										};
										$('#tb_departments')
												.bootstrapTable('refresh',
														url);
	...
	$("#sub").click(function() {
		var obj = {
			...
			//异步同步数据库
			$.getJSON("responsibilityDot/addresponsibilityDot", obj, function(result) {
					if (result.status == "success") {
						$("#add").modal("hide");
