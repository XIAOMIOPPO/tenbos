				...
				<div class="modal-body">
					<form class="form-horizontal" onsubmit="return check()" role="form">
						<div class="form-group">
							...
							<label for="lastname" class="col-sm-2 control-label">签收状态：</label>
							<div class="col-sm-4" id="printSignStatus">
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label">签收人：</label>
							<div class="col-sm-4" id="printAddresseeId">
							</div>
							<label for="lastname" class="col-sm-2 control-label">派送单位：</label>
							<div class="col-sm-4" id="printEmpName">
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label">受理单位：</label>
							<div class="col-sm-4" id="printPickup">
							</div>
							<label for="lastname" class="col-sm-2 control-label">结算方式：</label>
							<div class="col-sm-4" id="printSettlementMode">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-4" id="printProductAmount">
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label">体积：</label>
							<div class="col-sm-4" id="printSize">
							</div>
							<label for="lastname" class="col-sm-2 control-label">到达地：</label>
							<div class="col-sm-4" id="printAddresseeAddress">
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label">希望送达时间：</label>
							<div class="col-sm-4" id="printServiceTimeLimit">
							</div>
						...
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label">已配送：</label>
							<div class="col-sm-4" id="printDelivered">
							</div>
							<label for="lastname" class="col-sm-2 control-label">未配送：</label>
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			$('#tb_departments').bootstrapTable({
				url : 'delegateDetailed/delegateDetailedQueryWithCriteria', //请求后台的URL（*） 
				method : 'post', //请求方式（*） 
				...
				columns : [ {
					...
					title : '晚点率',
						...
						$('#btn_query').click(function() {
							var opt = {
								url : "delegateDetailed/delegateDetailedQueryWithCriteria",
								silent : true,
								query : {
									type : 1,
									level : 2,
					...
					var ButtonInit = function() {
						...
						if (select.length == 0) {
							...
							for (var i = 0; i < select.length; i++) {
								arrs.push(select[i].workSheetNo);
							} 
							$.ajax({
								url:"delegateDetailed/printDelegateDetailed",
								type:"post",
								data:{"workSheetNo":arrs},
								success:function(){
									window.open("jsps/TakeToSend/delegateDetailed/delegatePrint.jsp")
								}
							});
							var obj = select[0];
				            ...
				            var PrintSettlementMode = obj.settlementMode;
				            var printProduct = obj.product;
				            var PrintProductAmount = obj.productAmount;
				            var PrintSize = obj.size;
				            var PrintAddresseeAddress = obj.addresseeAddress;
				            var PrintServiceTimeLimit = obj.serviceTimeLimit;
				            ...
				            var printDelayRate = obj.delayRate;
					            ...
					            $("#printSettlementMode").text(printSettlementMode);
					            $("#printProduct").text(printProduct);
					            $("#printProductAmount").text(printProductAmount);
					            $("#printSize").text(printSize);
					            $("#printAddresseeAddress").text(printAddresseeAddress);
					            $("#printServiceTimeLimit").text(printServiceTimeLimit);
